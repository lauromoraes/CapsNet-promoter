...
def ConvNet(input_shape):
    ...
    conv1 = layers.Conv1D(filters=args.num_kernel1, kernel_size=args.kernel1_size, strides=args.kernel1_stride, padding='valid', activation='relu', name='conv1')(embed)
    # drop1 = layers.Dropout(0.5)(conv1)
    # pool1 = layers.MaxPooling1D(pool_size=args.pool1_size, strides=args.pool1_stride)(drop1)
    pool1 = layers.MaxPooling1D(pool_size=args.pool1_size, strides=args.pool1_stride)(conv1)

    flat1 = layers.Flatten()(pool1)

    dense1 = layers.Dense(10, activation='relu')(flat1)
...
def get_best_weight(args, actual_partition):
    ...
    for i in range(len(model_weights)):
        ...
        model = ConvNet(input_shape=x_train.shape)
...
if __name__ == "__main__":
    ...
    prefix_name = 'conv1_org_{}-batch_{}-emb_{}-kernel_{}-pool_{}'.format(args.organism, args.batch_size, args.emb, args.kernel1_size, args.pool1_size)
...
#    (x_train, y_train), (x_test, y_test) = load_imdb()
    ...
    kf = StratifiedShuffleSplit(n_splits=3, random_state=34267)
    kf.get_n_splits(X, y)
